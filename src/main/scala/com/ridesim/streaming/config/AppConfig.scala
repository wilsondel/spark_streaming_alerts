...
object AppConfig {
  ...
  def fromEnv(env: Map[String, String] = sys.env): AppConfig =
    AppConfig(
      ...
      topicLifecycle   = env.getOrElse("TOPIC_LIFECYCLE",     "topic.lifecycle"),
      ...
      jdbcUrl          = env.getOrElse("JDBC_URL",            "jdbc:postgresql://localhost:5432/ridesim"),
      jdbcUser         = env.getOrElse("JDBC_USER",           "postgres"),
      jdbcPassword     = env.getOrElse("JDBC_PASSWORD",       "")
    )
}
